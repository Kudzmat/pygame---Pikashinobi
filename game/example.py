...
# This creates the player and gives it the image found in this folder (similarly with the enemy images). 

player = pygame.image.load("image3.png")
enemy = pygame.image.load("shuriken.png")
enemy2 = pygame.image.load("boss.png") 
enemy3 = pygame.image.load("shurikens.png")
enemy4 = pygame.image.load("redshuriken.png")
prize = pygame.image.load("scroll.png")
enemy5 = pygame.image.load("pika1.png") 
...
enemy2YPosition =  random.randint(0, screen_height - enemy2_height)  # start at a particular place on map
...
keyUp= False
keyDown = False
keyRight = False  # moving right
keyLeft = False  # moving left
...
while 1: # This is a looping structure that will loop the indented code until you tell it to stop(in the event where you exit the program by quitting). In Python the int 1 has the boolean value of 'true'. In fact numbers greater than 0 also do. 0 on the other hand has a boolean value of false. You can test this out with the bool(...) function to see what boolean value types have. You will learn more about while loop structers later. 
    ...
    # This draws the remaining enemy and prize models on to the screen
    ...
    if enemyXPosition < 520 - enemy_width or enemy2XPosition < 520 - enemy_width:  # once the first wave of enemies has travelled halfway, the second wave will begin moving
        enemy3XPosition -= 1.80
        enemy4XPosition -= 1.70
        prizeXPosition -=1.30  # prize moves the slowest
        enemy5XPosition -= 1.50
    ...
    if keyDown == True:
        if playerYPosition < screen_height - image_height:# This makes sure that the user does not move the player below the window.
            playerYPosition += 1
    if keyRight == True:
        if playerXPosition < screen_width - 0:# This makes sure that the user moves to the right.
            playerXPosition += 1
    if keyLeft == True:
        if playerXPosition < screen_width - 0:# This makes sure that the user moves to left.
            playerXPosition -= 1
    ...
    if playerBox.colliderect(enemyBox) or playerBox.colliderect(enemy2Box) or playerBox.colliderect(enemy3Box) or playerBox.colliderect(enemy4Box) or playerBox.colliderect(enemy5Box):
    ...
    if playerBox.colliderect(prizeBox):
        ...
        print("Congratulations, you win and are now the strongest shinobi!")
    ...
    # If the prize gets off the screen the user loses the game:
    
    if prizeXPosition < 0 - prize_width:

        # Display losing status to the user: 
        
        print("You failed to capture the scroll, you lose!")
